...
export function updateAccountLinks(){const session=getSession(); const dashboardPath=getDashboardPath(); const accountLink=document.getElementById('accountLink'); if(accountLink){accountLink.textContent=session?`Olá, ${session.user.name.split(' ')[0]}`:'Entrar'; accountLink.href=session?dashboardPath:'/pages/login.html'} const miniUserArea=document.getElementById('miniUserArea'); if(miniUserArea){miniUserArea.innerHTML=session?`<span>${session.user.name}</span> <button id="logoutMini" class="ghost-btn" type="button">Sair</button>`:`<a class="ghost-btn" href="/pages/login.html">Entrar</a>`; const btn=document.getElementById('logoutMini'); if(btn) btn.onclick=()=>{clearSession(); location.href='/pages/login.html'}; }}
